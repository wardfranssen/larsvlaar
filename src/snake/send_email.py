...
with open('../../static/snake/images/equation.png', 'rb') as img:
    img_data = img.read()
    equation_image = MIMEImage(img_data, name='formula_image.png')
...
for i, email_address in enumerate(emails):
    # This means that we can only have as many workers as emails
    # If port is >= 8000 we are testing
    if app.port >= 8000:
        if i != app.port - 8000:
            print(app)
            continue
    else:
        # If port >= we are in prod
        if i != app.port - 5000:
            print(app)
            continue
...
def get_smtp_connection(email, password):
    ...
    smtp_connections[email] = smtplib.SMTP_SSL('smtp.gmail.com', 465)
    ...
    try:
        ...
        for email_data in emails:
            if email_data["EMAIL_ADDRESS"] == sender_email:
                smtp_connections[sender_email].connect()
...
def login_verification_email(verification_code: str, username:str, receiver: str):
    ...
    try:
        ...
        print("EMAIL NOT CONNECTED 12312312")
        for email_data in emails:
            if email_data["EMAIL_ADDRESS"] == sender_email:
